...
// NoDedupMetricsProducer is a metrics-only producer that disables resource/scope deduplication.
// This is used for benchmarking to measure the impact of columnar normalization on compression.
type NoDedupMetricsProducer struct {
	pool            memory.Allocator
	zstd            bool
	streamProducers map[string]*nodedupMetricsStreamProducer
	nextSchemaId    int64
	batchId         int64

	metricsBuilder       *NoDedupMetricsBuilder
	metricsRecordBuilder *builder.RecordBuilderExt

	stats *pstats.ProducerStats
}

type nodedupMetricsStreamProducer struct {
	output         bytes.Buffer
...
// NewNoDedupMetricsProducer creates a new metrics producer with resource/scope deduplication disabled.
func NewNoDedupMetricsProducer(options ...cfg.Option) *NoDedupMetricsProducer {
	...
	// Use NoDedupMetricsBuilder - disables resource/scope deduplication
	metricsBuilder, err := NewNoDedupMetricsBuilder(
		metricsRecordBuilder,
		metricsarrow.NewConfig(conf), // Keep sorting enabled
		stats,
		conf.Observer,
	)
	if err != nil {
	...
	return &NoDedupMetricsProducer{
		pool:                 conf.Pool,
		zstd:                 conf.Zstd,
		streamProducers:      make(map[string]*nodedupMetricsStreamProducer),
		batchId:              0,
...
func (p *NoDedupMetricsProducer) BatchArrowRecordsFromMetrics(metrics pmetric.Metrics) (*colarspb.BatchArrowRecords, error) {
	// Build main record
...
func (p *NoDedupMetricsProducer) buildMetricsRecord(metrics pmetric.Metrics) (arrow.Record, error) {
...
func (p *NoDedupMetricsProducer) produce(rms []*record_message.RecordMessage) (*colarspb.BatchArrowRecords, error) {
	...
	for i, rm := range rms {
		err := func() error {
			...
			if sp == nil {
				...
				sp = &nodedupMetricsStreamProducer{
					output:      buf,
...
func (p *NoDedupMetricsProducer) Close() error {
